# *************************************************************************
# Module: Makefile (adapted from english version 1.112)
...
pdf: $(XSL_PRINT) manual.xml $(SRCS) Makefile
	xsltproc  --xinclude -o $(BASENAME)$(VERSION)_manual-fr.fo ${XSL_PRINT} manual.xml
	fop.sh -fo $(BASENAME)$(VERSION)_manual-fr.fo -pdf $(BASENAME)$(VERSION)_manual-fr.pdf
	rm $(BASENAME)$(VERSION)_manual-fr.fo

pdfA4: $(XSL_PRINT) manual.xml $(SRCS) Makefile
	xsltproc --stringparam paper.type A4  --xinclude \
		-o $(BASENAME)$(VERSION)_manual-fr_A4.fo ${XSL_PRINT} manual.xml
	fop.sh -fo $(BASENAME)$(VERSION)_manual-fr_A4.fo -pdf $(BASENAME)$(VERSION)_manual-fr_A4.pdf
	rm $(BASENAME)$(VERSION)_manual-fr_A4.fo
...
htmlhelp: ${XSL_HTMLHELP} manual.xml $(SRCS)
	...
	mv ${HTMLHELP_DIR}/htmlhelp.chm csound_manual-fr.chm
...
html-onechunk-package:
	#tar -czf $(BASENAME)-html-$(VERSION).tar.gz csound-$(VERSION)/manual
	zip -r /tmp/$(BASENAME)$(VERSION)_manual-fr_html_single.zip singleHTML
	mv /tmp/$(BASENAME)$(VERSION)_manual-fr_html_single.zip ./

pdf-package:
	#tar -czf $(BASENAME)-html-$(VERSION).tar.gz csound-$(VERSION)/manual
	zip -r /tmp/$(BASENAME)$(VERSION)_manual-fr_pdf.zip $(BASENAME)$(VERSION)_manual-fr.pdf
	mv /tmp/$(BASENAME)$(VERSION)_manual-fr_pdf.zip ./

pdfA4-package:
	#tar -czf $(BASENAME)-html-$(VERSION).tar.gz csound-$(VERSION)/manual
	zip -r /tmp/$(BASENAME)$(VERSION)_manual-fr_A4_pdf.zip $(BASENAME)$(VERSION)_manual-fr_A4.pdf
	mv /tmp/$(BASENAME)$(VERSION)_manual-fr_A4_pdf.zip ./

htmlhelp-package:
	zip -r /tmp/$(BASENAME)$(VERSION)_manual-fr_winhelp_chm.zip csound_manual-fr.chm
