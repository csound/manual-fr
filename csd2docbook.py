...
class OrchestraTransform(object):
    ...
    def transformLines(self, lines):
        ...
        for s in lines:
            i = s.find(';')
            if i == 0:
                outLines.append(self.simpleComment(s))
            elif i > 0:
                outLines.append(self.orcLine(s[:i]) + self.simpleComment(s[i:]))
            else:
...
class CsdTransform(object):
    ...
    def write_playable_example(self, filename, text):
        try:
            ...
            print "Writing playable example:",  html_filename
...
def main():
    '''Usage: python csd2docbook options [filename]
